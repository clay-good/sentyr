...
test-cov:
	python3 -m pytest tests/ --cov=sentyr --cov-report=term-missing --cov-report=html

lint:
	python3 -m ruff check sentyr/
	python3 -m flake8 sentyr/ --max-line-length=100 --ignore=E203,W503

format:
	python3 -m black sentyr/ tests/
...
run-example:
	...
	python3 -m sentyr.cli analyze test_data/guardduty_crypto_mining.json \
		--source guardduty \
...
docker-stop:
	docker stop sentyr || true
	docker rm sentyr || true

docker-logs:
	docker logs -f sentyr
...
k8s-logs:
	kubectl logs -f deployment/sentyr -n $(NAMESPACE)

k8s-describe:
	kubectl describe deployment sentyr -n $(NAMESPACE)

k8s-scale:
	kubectl scale deployment sentyr --replicas=$(REPLICAS) -n $(NAMESPACE)

k8s-restart:
	kubectl rollout restart deployment/sentyr -n $(NAMESPACE)

k8s-port-forward:
	kubectl port-forward -n $(NAMESPACE) deployment/sentyr 8000:8000
...
# Development server
dev:
	python3 -m sentyr.cli serve --reload

# Backup
backup:
	@mkdir -p backups
	@tar czf backups/sentyr-backup-$$(date +%Y%m%d-%H%M%S).tar.gz outputs/ chroma_db/
