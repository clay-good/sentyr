# Sentyr Configuration Guide

## Overview

Sentyr uses a flexible configuration system that supports:
...
1. Default values (defined in `sentyr/config.py`)
...
All configuration can be set via environment variables with the `SENTYR_` prefix:

```bash
export SENTYR_ENVIRONMENT=production
export SENTYR_LOG_LEVEL=INFO
...
SENTYR_ENVIRONMENT=production python -m sentyr.cli serve

# Or specify config file explicitly
python -m sentyr.cli serve --config config/production.yaml
```
...
```bash
export ANTHROPIC_API_KEY=sk-ant-api03-...
export SENTYR_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/YOUR/WEBHOOK/URL
```
...
def load_secrets_from_aws():
 ...
 response = client.get_secret_value(SecretId='sentyr/production')
 secrets = json.loads(response['SecretString'])
 
 os.environ['ANTHROPIC_API_KEY'] = secrets['anthropic_api_key']
 os.environ['SENTYR_SLACK_WEBHOOK_URL'] = secrets['slack_webhook_url']
```
...
def load_secrets_from_vault():
 ...
 path='sentyr/production'
 )
 
 for key, value in secrets['data']['data'].items():
 os.environ[f'SENTYR_{key.upper()}'] = value
```

## Configuration Validation

Sentyr validates configuration on startup:
...
```python
from sentyr.config import load_config, load_config_from_file
...
docker run -d \
 ...
 -e SENTYR_ENVIRONMENT=production \
 -e SENTYR_LOG_LEVEL=INFO \
 sentyr:latest
```
...
python -c "from sentyr.config import load_config; print(load_config().to_dict())"
```
...
**Solution**: Set `SENTYR_DEBUG=false` or `debug: false` in config file
