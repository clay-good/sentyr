# Sentyr CLI Usage Guide

## Overview

Sentyr is a modular AI agent framework designed CLI-first for security teams. Each of the 6 AI agents can be invoked independently from the command line, making it perfect for integration into security workflows, SOAR platforms, and automation pipelines.
...
sentyr analyze test_data/guardduty_crypto_mining.json \
  --source guardduty \
  --output-json analysis.json \
  --output-html report.html

# Analyze Datadog security event
sentyr analyze test_data/datadog_data_exfiltration.json \
  --source datadog \
  --enable-rag \
  --enable-cache

# Batch process multiple events
sentyr batch test_data/ \
  --source guardduty \
...
sentyr incident-response incident.json --output response_plan.json
```
...
sentyr vuln-management vulnerability.json --output remediation.json
```
...
```bash
# Tune detection rule
sentyr detection-engineering detection.json --output tuning.json
```
...
sentyr brand-protection --domain sentyr.com --output permutations.json
```
...
sentyr security-questionnaire questionnaire.csv \
...
```bash
# Clear all cache
sentyr clear --clear-cache

# Clear expired entries only
sentyr clear --clear-expired
```
...
sentyr serve --host 0.0.0.0 --port 8000

# Development mode with auto-reload
sentyr serve --reload

# Production mode with multiple workers
sentyr serve --workers 4
```
...
RESPONSE=$(sentyr incident-response incident.json --output /tmp/response.json)
...
# In GitLab CI/CD
script:
  - sentyr vuln-management scan_results.json --output remediation.json
...
0 2 * * * sentyr detection-engineering /var/log/detections.json --output /var/log/tuning_$(date +\%Y\%m\%d).json
```
...
RISK_SCORE=$(sentyr analyze alert.json --output-json /tmp/analysis.json | jq '.risk_score')
...
```python
from sentyr.agents import AgentOrchestrator, WorkflowDefinition, WorkflowStep
...
5. **Monitor Metrics**: Regularly check `sentyr stats --metrics` for performance
...
sentyr analyze test.json --source guardduty --api-key "your-key"
```
...
# Verify installation
python -c "import sentyr; print(sentyr.__version__)"
```
...
- GitHub: https://github.com/clay-good/sentyr
