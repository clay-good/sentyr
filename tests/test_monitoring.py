...
from sentyr.core.monitoring.health import HealthChecker, HealthStatus
from sentyr.core.monitoring.metrics import Counter, Gauge, Histogram, MetricsCollector
from sentyr.core.monitoring.prometheus_exporter import PrometheusExporter
...
class TestHealthChecker:
    ...
    @patch("sentyr.core.monitoring.health.psutil")
    ...
    @patch("sentyr.storage.StateManager")
...
class TestPrometheusExporter:
    ...
    def test_record_scan_start(self):
        ...
        assert start_time > 0
        assert exporter.counters["sentyr_scans_total"].value >= 1
    ...
    def test_record_scan_end(self):
        ...
        assert exporter.counters["sentyr_files_scanned_total"].value >= 100
        assert exporter.counters["sentyr_issues_found_total"].value >= 5
    ...
    def test_record_pii_detection(self):
        ...
        assert exporter.counters["sentyr_pii_detections_total"].value >= 3
    ...
    def test_record_api_request(self):
        ...
        assert exporter.histograms["sentyr_api_request_duration_seconds"].get_count() >= 1
    ...
    def test_record_api_request_error(self):
        ...
        assert exporter.counters["sentyr_api_errors_total"].value >= 1
    ...
    def test_record_error(self):
        ...
        assert exporter.counters["sentyr_errors_total"].value >= 1
    ...
    def test_record_alert(self):
        ...
        assert exporter.counters["sentyr_alerts_sent_total"].value >= 1
    ...
    def test_update_cache_metrics(self):
        ...
        assert exporter.gauges["sentyr_cache_hit_rate"].value == 0.85
        assert exporter.gauges["sentyr_cache_size"].value == 100.0

    def test_export_metrics(self):
        """Test exporting metrics in Prometheus format."""
        exporter = PrometheusExporter()
        exporter.inc_counter("sentyr_scans_total", 5)

        output = exporter.export_metrics()

        assert "# HELP" in output
        assert "# TYPE" in output
        assert "sentyr_scans_total" in output
        assert "sentyr_info" in output
        assert "sentyr_uptime_seconds" in output
