"""Command-line interface for Sentyr."""
...
from sentyr.output import OutputFormatter
from sentyr.html_report import HTMLReportGenerator
from sentyr.cache import AnalysisCache
from sentyr.batch import BatchProcessor
...
def analyze(
    ...
    # Display header
    click.secho(f"\n{'='*60}", fg='cyan')
    click.secho("  SENTYR SECURITY ANALYST", fg='cyan', bold=True)
    click.secho(f"{'='*60}\n", fg='cyan')
...
def serve(host: str, port: int, reload: bool, workers: int, api_key: Optional[str]) -> None:
    ...
    click.secho("  SENTYR API SERVER", fg='cyan', bold=True)
    click.secho("============================================================", fg='cyan', bold=True)
    click.echo("")
...
def list_agents() -> None:
    ...
    for agent in agents_info:
        ...
        click.echo(f"  Command: sentyr {agent['command']}")
    ...
    click.secho("Use 'sentyr <command> --help' for more information on each agent.\n", fg='yellow')
