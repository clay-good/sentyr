"""
PagerDuty Integration for Sentyr.
...
@dataclass
class SyncMapping:
    """Mapping between Sentyr incident and PagerDuty incident."""
    incident_id: str
    pd_incident_id: str
    pd_incident_number: int
    dedup_key: str
    last_synced: datetime
    sync_direction: str  # "sentyr_to_pd", "pd_to_sentyr", "bidirectional"
...
class PagerDutyIncidentManager:
    ...
    def _map_severity_to_pd_severity(self, severity: Severity) -> PagerDutySeverity:
        """Map Sentyr severity to PagerDuty severity."""
        mapping = {
    ...
    async def trigger_incident_from_sentyr(
        self,
        incident: Incident,
        analysis: Optional[AnalysisResult] = None
    ) -> Optional[Dict[str, Any]]:
        """Trigger PagerDuty incident from Sentyr incident."""
        try:
            severity = self._map_severity_to_pd_severity(incident.severity)
            custom_details = self._build_custom_details(incident, analysis)
            dedup_key = f"sentyr-{incident.incident_id}"
            ...
            if result:
                ...
                logger.info(f"Triggered PagerDuty incident for Sentyr incident {incident.incident_id}")
    ...
    async def sync_incident_to_pagerduty(
        ...
        """Sync Sentyr incident updates to PagerDuty."""
        try:
            ...
            if success:
                ...
                logger.info(f"Synced Sentyr incident {incident.incident_id} to PagerDuty")
