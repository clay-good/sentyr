...
class MetricsCollector:
    ...
    def record_analysis(
        ...
        with self._lock:
            ...
            # Record as metrics
            self.increment_counter("sentyr_analyses_total", labels={"platform": platform})
            self.observe_histogram("sentyr_analysis_duration_seconds", latency_seconds, {"platform": platform})
            self.observe_histogram("sentyr_risk_score", risk_score, {"platform": platform})
            self.increment_counter("sentyr_tokens_total", tokens_used, {"platform": platform})
    ...
    def record_api_request(self, endpoint: str, method: str, status_code: int, latency_seconds: float) -> None:
        ...
        with self._lock:
            ...
            self.increment_counter("sentyr_api_requests_total", labels=labels)
            self.observe_histogram("sentyr_api_duration_seconds", latency_seconds, labels)
    ...
    def export_prometheus(self) -> str:
        ...
        metrics_meta = [
            ("sentyr_analyses_total", "counter", "Total number of security analyses performed"),
            ("sentyr_analysis_errors_total", "counter", "Total number of analysis errors"),
            ("sentyr_cache_hits_total", "counter", "Total number of cache hits"),
            ("sentyr_cache_misses_total", "counter", "Total number of cache misses"),
            ("sentyr_tokens_total", "counter", "Total number of AI tokens used"),
            ("sentyr_cost_usd_total", "counter", "Total cost in USD"),
            ("sentyr_high_risk_events_total", "counter", "Total high-risk events (score >= 7)"),
            ("sentyr_analysis_duration_seconds", "histogram", "Analysis duration in seconds"),
            ("sentyr_risk_score", "histogram", "Risk score distribution"),
        ]
        ...
        lines.append(f"sentyr_analyses_total {self.analysis_count}")
        lines.append(f"sentyr_analysis_errors_total {self.analysis_errors}")
        lines.append(f"sentyr_cache_hits_total {self.cache_hits}")
        lines.append(f"sentyr_cache_misses_total {self.cache_misses}")
        lines.append(f"sentyr_tokens_total {self.total_tokens_used}")
        lines.append(f"sentyr_cost_usd_total {self.total_cost_usd:.4f}")
        lines.append(f"sentyr_high_risk_events_total {self.high_risk_events}")

        # Export by platform
        for platform, count in self.events_by_platform.items():
            lines.append(f'sentyr_analyses_total{{platform="{platform}"}} {count}')

        # Export histogram summaries
        if self.analysis_latencies:
            sorted_latencies = sorted(self.analysis_latencies)
            lines.append(f"sentyr_analysis_duration_seconds_sum {sum(sorted_latencies):.3f}")
            lines.append(f"sentyr_analysis_duration_seconds_count {len(sorted_latencies)}")

        if self.risk_scores:
            lines.append(f"sentyr_risk_score_sum {sum(self.risk_scores):.2f}")
            lines.append(f"sentyr_risk_score_count {len(self.risk_scores)}")
