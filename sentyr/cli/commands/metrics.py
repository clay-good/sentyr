...
from sentyr.core.monitoring.prometheus_exporter import get_exporter
...
def export(output, format):
    ...
    Examples:
        # Export to stdout
        sentyr metrics export
        
        # Export to file
        sentyr metrics export --output metrics.txt
        
        # Export as JSON
        sentyr metrics export --format json
    """
    try:
...
def show(watch):
    ...
    try:
        while True:
            ...
            table = Table(title="Sentyr Metrics", show_header=True, header_style="bold magenta")
...
def reset():
    ...
    """
    try:
        from sentyr.core.monitoring.prometheus_exporter import reset_exporter
...
def serve(port, host):
    ...
    Examples:
        ...
        sentyr metrics serve
        
        # Start server on custom port
        sentyr metrics serve --port 8080
        
        # Bind to localhost only
        sentyr metrics serve --host 127.0.0.1
    """
    try:
