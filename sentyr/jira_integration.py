...
@dataclass
class SyncMapping:
    """Mapping between Sentyr incident and Jira issue."""
    incident_id: str
    jira_key: str
    jira_id: str
    last_synced: datetime
    sync_direction: str  # "sentyr_to_jira", "jira_to_sentyr", "bidirectional"
...
class JiraIssueManager:
    ...
    def _map_severity_to_priority(self, severity: Severity) -> JiraPriority:
        """Map Sentyr severity to Jira priority."""
        mapping = {
    ...
    def _build_issue_description(
        ...
        """Build Jira issue description from Sentyr incident."""
        lines = [
            "h2. Sentyr Incident",
    ...
    async def create_issue_from_sentyr(
        self,
        incident: Incident,
        analysis: Optional[AnalysisResult] = None
    ) -> Optional[JiraIssue]:
        """Create Jira issue from Sentyr incident."""
        try:
            ...
            if jira_issue:
                ...
                logger.info(f"Created Jira issue {jira_issue.key} for Sentyr incident {incident.incident_id}")
    ...
    async def sync_incident_to_jira(
        ...
        """Sync Sentyr incident updates to Jira."""
        try:
            priority = self._map_severity_to_priority(incident.severity)
            ...
            if jira_issue:
                ...
                logger.info(f"Synced Sentyr incident {incident.incident_id} to Jira {jira_issue.key}")
